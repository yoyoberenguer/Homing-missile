...
try:
    ...
    from pygame import Rect, transform, surfarray, BLEND_RGB_ADD, HWSURFACE, display
except ImportError:
...
# @cython.nonecheck(False)
# @cython.cdivision(True)
# cdef projection_degrees(point_, rect_, float angle_):
#     """
#
#     :param point_: Vector2
#     :param rect_ : Vector2
#     :param angle_: float;
#     :return:
#     """
#
#     cdef float p_angle = -<float>atan2(point_.y, point_.x) * RAD_TO_DEG
#     cdef float width = point_.width()
#     cdef float rotation = DEG_TO_RAD * ((p_angle % 360 + angle_ - 90) % 360)
#     return Vector2(rect_.x + cos(rotation) * width, rect_.y - sin(rotation) * width)
...
# @cython.nonecheck(False)
# @cython.cdivision(True)
# cdef float convert_pi_0_2pi(float rad)nogil:
#     return (rad + M_2PI) % M_2PI
...
cdef class MParticleFX(object):
    ...
    def __init__(self, gl_, rect_, vector_, int layer_, int angle_,
        ...
        self.n = <int>round(self.length_1 / 19.0)
