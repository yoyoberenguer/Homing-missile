...
#define M_PI2_ 3.14159265358979323846/2.0
#define RAD_TO_DEG_ 180.0/M_PI_
#define DEG_TO_RAD_ M_PI_/180.0
...
{
 double rad;
 rad=(atan2(v->y, v->x) * (float)RAD_TO_DEG_ + deg) * DEG_TO_RAD_;
 v->x = (float)cos(rad);
 v->y = (float)sin(rad);
}
...
float angle_to(struct vector2d v1, struct vector2d v2)
{
 ...
 return (float)((v2_rad - v1_rad) * (float)RAD_TO_DEG_);
}
...
{
 return (float)((float)atan2(v.y, v.x) * (float)RAD_TO_DEG_);
}
...
struct mla_pack trajectory(struct vector2d p1, struct vector2d p2, struct vector2d v1, struct vector2d v2)
{
  ...
  // float angle = atan2(vector.y, vector.x) * (float)RAD_TO_DEG_;     // --> this is not used
